...
ballRadius = 2.0
x1 = [1.5, 1.0] + ballRadius * np.random.randn(NBVECS, VECDIM)
x2 = [1.3, 1.8] + ballRadius * np.random.randn(NBVECS, VECDIM)
x3 = [0, 1.4] + ballRadius * np.random.randn(NBVECS, VECDIM)
...
y_pred = gnb.predict([[1.3, 1.8]])
print(y_pred)

y_pred = gnb.predict([[0, 1.4]])
print(y_pred)
...
plt.text(1.3, 1.8, "B", verticalalignment='center', horizontalalignment='center', fontproperties=font)
plt.text(0, 1.4, "C", verticalalignment='center', horizontalalignment='center', fontproperties=font)
scatter(x1[:, 0], x1[:, 1], s=1.0, color='#FF6B00')
scatter(x2[:, 0], x2[:, 1], s=1.0, color='#95D600')
scatter(x3[:, 0], x3[:, 1], s=1.0, color='#00C1DE')
r.savefig('Naivefig.jpeg')
plt.close(r)
show()
